...
public sealed class JwtTokenGenerator : IJwtTokenGenerator
{
    ...
    public (string AccessToken, DateTime ExpiresAtUtc) GenerateAccessToken(User user, IReadOnlyCollection<string> roles)
    {
        ...
        if (string.Equals(user.RequestedRoleCode, GuestRoleCode, StringComparison.OrdinalIgnoreCase)
            && user.IdentityAccessExpiresAtUtc is { } identityExpiryUtc)
        {
            var normalizedIdentityExpiryUtc = identityExpiryUtc.Kind == DateTimeKind.Utc
                ? identityExpiryUtc
                : identityExpiryUtc.ToUniversalTime();

            if (normalizedIdentityExpiryUtc < expiresAtUtc)
            {
                expiresAtUtc = normalizedIdentityExpiryUtc;
            }
        }
